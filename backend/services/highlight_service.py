...
async def run_compare_highlight(
    ...
    validate_pdf_bytes(compare_bytes, compare_file.filename)
    ...
    # BERT embedding + PDF annotasyonu CPU-ağır — event loop'u bloklamasın.
    pairs = await loop.run_in_executor(
        None, partial(_bert_sentence_pairs, target_raw, compare_raw, threshold=0.80, top_n=top_words)
    )
    ...
    target_hl = await loop.run_in_executor(
        None, partial(_highlight_two_tiers, target_bytes, identical_t, semantic_t)
    )
    compare_hl = await loop.run_in_executor(
        None, partial(_highlight_two_tiers, compare_bytes, identical_c, semantic_c)
    )
