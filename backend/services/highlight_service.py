...
COLOR_IDENTICAL = (1.0, 0.45, 0.0)   # parlak turuncu  — BERT >= 0.95
COLOR_SEMANTIC  = (1.0, 0.82, 0.0)   # altın sarısı    — BERT 0.80–0.95
...
def _bert_sentence_pairs(text1: str, text2: str, threshold: float = 0.80, top_n: int = 60):
    """BERT ile iki metin arasındaki anlamsal benzer cümle çiftlerini döndürür (1-1 eşleştirme)."""
...
def _highlight_two_tiers(file_bytes: bytes, tier1: list[str], tier2: list[str]) -> bytes:
    """tier1 = özdeş (turuncu), tier2 = semantik benzer (sarı)"""
    doc = fitz.open(stream=file_bytes, filetype="pdf")
    for page in doc:
        _annotate_page(page, tier2, COLOR_SEMANTIC)    # önce sarı (altta)
        _annotate_page(page, tier1, COLOR_IDENTICAL)   # sonra turuncu (üstte)
    result = doc.tobytes()
    doc.close()
    return result


async def run_compare_highlight(target_file, compare_file) -> dict:
    from services.text_preprocessing import extract_full_text_from_pdf

    target_bytes  = await target_file.read()
    compare_bytes = await compare_file.read()

    target_raw  = extract_full_text_from_pdf(target_bytes)
    compare_raw = extract_full_text_from_pdf(compare_bytes)

    pairs = _bert_sentence_pairs(target_raw, compare_raw, threshold=0.80, top_n=60)
